...
void f_recover(char input[]) {
	...
	strncpy(last_locdir, buf, cnt+1); //원래 파일이 있던 위치를 탐색
	
	if((count = scandir(last_locdir, &namelist2, dir_skip, alphsort)) < 0) {
	...
	for(int i = 0; i < count; i++) {
		...
		if(!strcmp(fname, namelist2[i]->d_name)) {
			idc = 1;
			break;
		}
	...
	if(idc == 0) { //이름이 겹치는 파일이 없을떄
		...
		frontnum = same_name_max2(fname, last_locdir);
		frontnum++;
